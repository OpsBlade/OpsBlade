...
const taskID = "aws_ec2_instance_start"

const startResponse = `<StartInstancesResponse xmlns="http://ec2.amazonaws.com/doc/2016-11-15/">
...
func TestExecute(t *testing.T) {
	cases := []struct {
		...
		{
			name: "started", status: http.StatusOK, body: startResponse,
			wantOK: true, wantMsg: "AWS EC2 Instance started", wantData: map[string]any{"instance_id": "i-0123"},
		},
		{
			...
			wantMsg: "failed to start instance: ",
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := awstest.Run(t, taskID, shared.TaskContext{DryRun: tc.dryRun, Debug: true}, map[string]any{"instance_id": "i-0123"})
			assert.Equal(t, tc.wantOK, r.Success)
			assert.Empty(t, r.OnFail)
			assert.Contains(t, r.Msg, tc.wantMsg)
			if !tc.wantOK {
				assert.Contains(t, r.Msg, "api error")
			}
			if tc.wantData == nil {
			...
			forms := rec.Forms("StartInstances")
			require.Len(t, forms, 1)
			assert.Equal(t, "i-0123", forms[0].Get("InstanceId.1"))
			if tc.dryRun {
				assert.Equal(t, "true", forms[0].Get("DryRun"))
			} else {
				assert.Equal(t, "false", forms[0].Get("DryRun"))
			}
		})
	}
}
...
func TestExecute_BadInstructions(t *testing.T) {
	...
	ctx := shared.TaskContext{Task: taskID, Instructions: []byte(`{"instance_id": 42}`)}
